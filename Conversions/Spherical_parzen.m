...
points_likelihood=zeros(800,8);
    for pts=1:800
        for cls=1:8
            ...
            disp(temp);
            for b=1:500
                dist_vector=test{1,temp}(rem(pts-1,100)+1,:)-train{1,cls}(b,:);
...
for pts=1:800
    ...
    max_ind=1;
    for cls=1:8
        output_weights(cls,pts)=points_likelihood(pts,cls);
        if(max_val<points_likelihood(pts,cls))
            max_val=points_likelihood(pts,cls);
            max_ind=cls;
        end
    end
    %output_weights(max_ind,pts)=1;
    %output_classes(max_ind,pts)=1;
    max(1,pts)=max_val;
    %if(max_ind==floor((pts-1)/100)+1)
        ac(1,pts)=1;
    %end
...
plotconfusion(actual_classes,output_weights);
disp(points_likelihood);